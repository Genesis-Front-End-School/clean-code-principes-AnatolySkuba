...
import { CourseLesson, LessonStatus } from '../../entities/course';

jest.mock('react-query', () => ({
  useQuery: jest.fn(),
}));

jest.mock('../../entities/course', () => ({
...
describe('CourseDetails', () => {
  ...
  it('shows h1 when an unlocked lesson is clicked', () => {
    expect(wrapper.find('VideoPlayer').prop('src')).toBe(mockCourse.meta.courseVideoPreview.link);

    wrapper.find('li').first().simulate('click');

    expect(wrapper.find('VideoPlayer').prop('src')).toBe(mockCourse.lessons[0].link);
