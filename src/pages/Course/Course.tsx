...
export const Course = (): JSX.Element => {
  ...
  if (
    ...
    const hls = new Hls({
      xhrSetup: (xhr) => {
        ...
        xhr.setRequestHeader('Authorization', `Bearer ${localStorage.getItem(STORAGE_KEYS.TOKEN)}`);
      },
    });
  ...
  const pip = async (id: string) => {
    ...
    const eventListenerKeydown = (event: KeyboardEvent) => {
      if (event.ctrlKey) {
        event.code === 'Comma' &&
          (video.playbackRate = video.playbackRate >= 0.5 ? video.playbackRate - 0.25 : 0.25);
        event.code === 'Period' &&
          (video.playbackRate = video.playbackRate <= 3.75 ? video.playbackRate + 0.25 : 4);
      }
    };
          ...
          <ul>
            {lessons?.map((lesson: Lesson) => (
              <li key={lesson.id} className="flex items-center gap-3">
                ...
                {lesson.id === isPictureInPicture && (
                  <>
                    <FcStart className="animate-pulse" />
                    <p>
                      Press Ctrl + &#62; to increase speed, press Ctrl + &lt; to slow down playback
                    </p>
                  </>
                )}
              </li>
